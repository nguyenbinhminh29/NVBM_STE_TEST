...
5. **Động cơ Khuyến mãi (Promotion Engine):** Hệ thống không tính toán giảm giá tĩnh (static) tại Client, thay vào đó sở hữu một Promotion Microservices cho phép tính toán các rule bán hàng linh hoạt (VD: Buy 1 Get 1, Buy X% off Y) và áp dụng thời gian thực ngay khi quét giá POS, thiết kế theo pattern dạng Strategy dễ dàng gắn thêm luật kinh doanh mới.
...
### Khởi động dự án
Hệ thống sử dụng dự án **.NET Aspire (AppHost)** để làm nhạc trưởng (Orchestrator) quản lý toàn bộ quá trình build và chạy của các Microservices:

1. Mở PowerShell hoặc Terminal tại **thư mục gốc `Projects`**.
2. Thực thi lệnh khởi động Aspire:
   ```bash
   dotnet run --project src/NVBM.AppHost/NVBM.AppHost.csproj
   ```
3. Sau khi chạy, Hệ thống sẽ sinh ngẫu nhiên một đường dẫn bảo mật trỏ tới **Aspire Dashboard** (ví dụ: `https://localhost:17076`). Truy cập vào giao diện web này để xem danh sách toàn bộ các Web API đang chạy, biểu đồ CPU/RAM, các Log tập trung và flow Traces giữa các dịch vụ.
4. Mọi tương tác nghiệp vụ từ thế giới bên ngoài sẽ được cấu hình đi qua **YARP API Gateway** (địa chỉ lấy từ thông báo của Gateway trên Console, thường là `https://localhost:7122`).
